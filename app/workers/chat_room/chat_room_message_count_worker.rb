...
class ChatRoom::ChatRoomMessageCountWorker
  ...
  def perform
    ...
    ChatRoom.where(id: apps_to_update_ids).update_all(messages_count: "SELECT COUNT(*) FROM chat_messages WHERE chat_messages.chat_room_id = chat_rooms.id") if apps_to_update_ids.present?
